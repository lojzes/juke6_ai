...
class Server:
    ...
    async def read_resource(self, resource_uri: str,
        ...
        while attempt < retries:
            try:
                logger.info(f"Executing {resource_uri}...")
            ...
            # 达到最大重试次数时抛出异常
                ...
                attempt += 1
                logger.warning(f"Error executing resource: {e}. Attempt {attempt} of {retries}.")
                if attempt < retries:
                    logger.info(f"Retrying in {delay} seconds...")
                    await asyncio.sleep(delay)
                else:
            ...
            try:
                ...
                if self.stdio_context:
                    try:
                        ...
                        logger.info(f"Note: Normal shutdown message for {self.name}: {e}")
                    except Exception as e:
                        logger.warning(f"Warning during stdio cleanup for {self.name}: {e}")
                    finally:
            ...
            if "tool" in llm_call and "arguments" in llm_call:
                ...
                for server in self.servers:
                    try:
                        ...
                        logger.error(f"Server {server.name} does not have 'list_tools' method: {e}")
                        continue
                return f"No server found with tool: {llm_call['tool']}"

            # 2、如果响应包含资源名称和URI，执行读取相应的资源
            if "resource" in llm_call:
                logger.info(f"Executing resource: {llm_call['resource']}")
                ...
                for server in self.servers:
                    try:
                        ...
                        logger.error(f"Server {server.name} does not have 'list_resources' method: {e}")
                        continue
