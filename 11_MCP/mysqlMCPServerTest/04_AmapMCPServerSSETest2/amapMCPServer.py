...
async def run_agent():
    ...
    client = MultiServerMCPClient({
        # 高德地图MCP Server
        "amap-amap-sse": {
            "url": "https://mcp.amap.com/sse?key=019f32d5fb64063218476da9255d313c",
            "transport": "sse",
        },
        # 自定义 MCP server
        "calculator": {
    ...
    system_message = SystemMessage(content=(
        "你是一个AI助手，要充分理解用户的提问，然后回答问题。\n\n"
        "你已链接到以下MCP Server：\n"
        "1. 高德地图MCP Server（地理信息相关）\n"
        "2. 计算器MCP Server（数学计算相关）\n"
        "3. mysql_mcp_server（数据库查询相关，可获取student_info表等数据库数据）\n\n"
        "当用户提出查询数据、学生信息相关问题时，必须优先使用mysql_mcp_server提供的工具进行查询，"
        "先验证表是否存在，再获取表数据，若工具调用出错需反馈具体原因。"
    ))
